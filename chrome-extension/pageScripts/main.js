...
let ajax_interceptor_qoweifjqon = {
  ...
  myXHR: function () {
    ...
    const modifyResponse = () => {
      ajax_interceptor_qoweifjqon.settings.requestMock_rules.forEach(
        ({filterType = 'normal', switchOn = true, match, overrideTxt = '', overrideStatus}) => {
          let matched = false
          if (switchOn && match) {
          ...
          if (matched) {
            let originalResponse = this.response
            if (overrideTxt) {
            ...
            if (!pageScriptEventDispatched) {
              window.dispatchEvent(new CustomEvent('pageScript', {
                detail: {url: this.responseURL, match, originalResponse}
              }))
      ...
      if (attr === 'onreadystatechange') {
        xhr.onreadystatechange = (...args) => {
          if (this.readyState === 4) {
            // 请求成功
            if (ajax_interceptor_qoweifjqon.settings.requestMock_switchOn) {
              // 开启拦截
              modifyResponse()
            }
          }
...
window.addEventListener('message', function (event) {
  ...
  if (data.type === 'requestMock' && data.to === 'pageScript') {
